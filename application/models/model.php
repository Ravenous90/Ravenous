<?php
	class Model {
		...
		$gender, $phone, $age, $info, $db, $result, $myrow, $activation, $result_edit;
		
		function isEmptyFieldsSignup($username, $email, $password) { // проверка на пустые поля формы регистрации
		...
		function isEmptyFieldsLogin($username, $password) { // проверка на пустые поля формы входа
		...
		function cleanFieldsSignup($username, $email, $password) { // чистка данных полей формы регистрации и присвоение их переменным
		...
		function cleanFieldsLogin($username, $password) { // чистка данных полей формы входа и присвоение их переменным
			$username = stripcslashes($username);
			$username = htmlspecialchars($username);
			$username = trim($username);
					$password = stripcslashes($password);
		...
		function lengthUsername() { // проверка длины поля имя пользователя
		...
		function lengthPassword() { // проверка длины поля пароль
		...
		function isValidateEmail() { // проверка на валидность email
		...
		function testLogin() { // проверка: 1.на существование имени пользователя в БД 2.на активацию пользователя 3.на совпадение пароля. Вызов записи сессии
		...
		function sendConfirmMessage() {  // генерирование уникального кода, отправка письма с кодом на почту
			include "config.php";
		...
		function isUsernameOrEmailExist() { // проверка на существование имени пользователя и email в БД при регистрации
			include "db.php";
		...
		function insertSignup() { // добавление имени пользователя, email и пароля в БД после формы регистрации
			include "db.php";
		...
		function selectRow() { // выбор строки с БД по имени пользователя
			include "db.php";
		...
		function selectRowByEmail($set_email) { // выбор строки с БД по email
			include "db.php";
		...
		function updateInfo($set_username, $set_email, $set_gender, $set_age, $set_phone, $set_info) { // добавление/обновление информации в БД из формы личного кабинета
			include "db.php";
				if ($set_username != '') { $result_edit = mysql_query("UPDATE users SET username='$set_username' WHERE email='$set_email'"); }
				if ($set_gender != '') { $result_edit = mysql_query("UPDATE users SET gender='$set_gender' WHERE email='$set_email'"); }
				if ($set_age != '') { $result_edit = mysql_query("UPDATE users SET age='$set_age' WHERE email='$set_email'"); }
				if ($set_phone != '') { $result_edit = mysql_query("UPDATE users SET phone='$set_phone' WHERE email='$set_email'"); }
				if ($set_info != '') { $result_edit = mysql_query("UPDATE users SET info='$set_info' WHERE email='$set_email'"); }
				
					$this->result_edit = $result_edit;
		}
		
		function updatePassword($set_old_password, $set_new_password, $set_email) { // проверка введенного из формы личного кабинета пароля с паролем с БД 
		...
		function isOkUpdate() { // проверка на добавление/обновление информации введенной из формы личного кабинета
			if ($this->result_edit) {
		...
		function getActivationFromUrl() { // получение имени пользователя и кода активации из URL
		...
		function compareCodes() { // сравнение кодов активации
			$salt = "834u50g0aUdLdfa22opwMgx3z";
		...
		function updateVerification() { // обновление отметки об активации пользователя в БД
			include "db.php";
				mysql_query("UPDATE users SET verification='1' WHERE username='$this->username'");
		}
		
		function recordSession() { // запись сессии и перенаправление в личный кабинет
		...
		function updateSession($set_username, $set_email, $set_gender, $set_age, $set_phone, $set_info) { // обновление сессии после редактирования
			session_start();
